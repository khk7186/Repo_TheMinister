...
public class DebateMain : MonoBehaviour
{
    ...
    public List<Character> EnemyACharacterList;
    public List<Character> EnemyBCharacterList;
    public List<DebateTopic> debateTopicList = new List<DebateTopic>();
    public int currentTopicIndex = 0;
    public IEnumerator StartDebate()
    {
        bool gameNotOver = debateTopicList.Count != 0 && PlayerCharacters.Count != 0;
        TopicUI topicUI = FindObjectOfType<TopicUI>();
        while (gameNotOver)
        {
            if (debateTopicList[currentTopicIndex].IsClose == true)
            {
                currentTopicIndex++;
                yield return topicUI.StartNewTopic(debateTopicList[currentTopicIndex]);
            }
        }
