...
public class Character : MonoBehaviour, IRound
{
    ...
    public void TryMergeTags()
    {
        foreach (var suspect in Player.MergeTagDict)
        {
            ...
            Debug.Log(string.Join(",", Player.MergeTagDict[Tag.武忠]));
            for (int i = 0; i < Player.MergeTagDict[Tag.武忠].Count; i++)
            {
                Debug.Log($"removed: {Player.MergeTagDict[Tag.武忠][i]}");
                tagList.Remove(Player.MergeTagDict[Tag.武忠][i]);
            }
