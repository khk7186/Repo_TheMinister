...
#region constants
...
#if URP12_OR_NEWER
    readonly GUIContent sourceBufferLabel = new GUIContent("Source Buffer (URP 12+)",
                                                           "Change if the effect does not work in URP 12+. Selection depends on rendering settings");
#endif
    ...
    public override void OnInspectorGUI()
    {
        ...
        {
            EditorGUILayout.PrefixLabel(sourceBufferLabel);
            GUILayout.Space(EditorGUIUtility.pixelsPerPoint * 2);
            tiSource.sourceBuffer = (TranslucentImageSource.SourceBuffer)GUILayout.Toolbar(
                (int)tiSource.sourceBuffer,
                new[] {
                    nameof(TranslucentImageSource.SourceBuffer.A),
                    nameof(TranslucentImageSource.SourceBuffer.B)
                },
                GUILayout.ExpandWidth(false),
                GUILayout.Width(EditorGUIUtility.singleLineHeight * 3f)
            );
        }
#endif
